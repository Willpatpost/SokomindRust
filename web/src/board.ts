...
export class BoardView {
  ...
  draw(width: number, height: number, tiles: Uint8Array, labels: Uint8Array, state: Snapshot, onGoal: boolean[]) {
    ...
    c.fillStyle = '#101b21'; c.fillRect(0, 0, pixelWidth / ratio, pixelHeight / ratio);
    ...
    const label = `${width} by ${height} puzzle, ${labels.length} boxes. ${state.moves} moves, ${state.pushes} pushes.${state.solved ? ' Solved.' : ''} Use arrow keys or WASD.`;
