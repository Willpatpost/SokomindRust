...
export type SearchStatus = 'running' | 'solved' | 'exhausted' | 'state_limit' | 'memory_limit' | 'time_limit' | 'cancelled';
export type Proof = { kind: 'optimal'; moves: number } | { kind: 'bounded'; lower: number; upper: number } | { kind: 'unsolvable' } | { kind: 'none' };
/** `best` is the best route's move count and `lowerBound` the live certified bound, when known. */
export interface Metrics { expanded: number; generated: number; reservedBytes: number; best?: number; lowerBound?: number; proof: Proof; status: SearchStatus }
